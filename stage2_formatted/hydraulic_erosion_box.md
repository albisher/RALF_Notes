**Tags:** #physics-simulation, #geomorphology, #hydraulic-erosion, #procedural-generation, #box-system
**Created:** 2026-01-13
**Type:** code-implementation

# hydraulic_erosion_box
...
Initializes the box with supported world types and metadata.

### ``execute(input_data)``

Core logic: validates input, checks world compatibility, builds neighbor relationships, and runs erosion simulation.

### ``_build_neighbor_map(cells)``

Constructs adjacency data for each cell to identify downhill neighbors.

### ``simulate_droplet_flow()``

(Implicitly handled in `execute`): Deterministically places droplets, tracks erosion/deposition, and records river paths.
