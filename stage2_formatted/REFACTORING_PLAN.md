**Tags:** #refactoring, #object-oriented-design, #code-organization, #box-based-architecture, #codebase-improvement, #modularization, #state-management, #composable-components
**Created:** 2026-01-12
**Type:** documentation
...
Core orchestrator managing the application’s high-level workflow.

### `DroneState.js`

Manages drone-specific state (e.g., position, status) in a centralized way.

### `DroneService.js`

Handles drone business logic (e.g., API interactions, validation).

### `DroneComputed.js`

Computes derived drone properties (e.g., derived metrics, UI updates).

### `useDrones.js`

Vue.js composable for drone-related reactivity and lifecycle hooks.

### `AppState.js`

Global application state container (e.g., active views, session data).
...
> `Vue.js (for composables and reactivity)`
> `no external libraries explicitly listed (focused on internal modularization).`

## Related

- [[Codebase_Architecture_Diagram]]
- [[Vue_Composition_API_Guide]]
