**Tags:** #automated-testing, #web-scraping, #puppeteer, #ui-testing, #e2e-testing
**Created:** 2026-01-13
**Type:** code-notes
...
Sets up directory structure and initializes logging for the test suite.

### ``getScreenshotName(feature, description)``

Generates a unique filename for screenshots combining timestamp, counter, feature name, and sanitized description.

### ``takeScreenshot(page, feature, description)``

Captures a full-page screenshot of a Puppeteer page and saves it to disk with error handling.

### ``testBasicConnection(page)``

Verifies the application's basic connectivity by checking HTTP response status and page title.

### ``testLoginFlow(page)``

Automates the login process, dynamically locates form elements, submits credentials, and checks for successful navigation to protected pages.
