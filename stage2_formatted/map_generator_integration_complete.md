**Tags:** #integration, #map-generation, #world-system, #ui-components, #box-system, #api-integration
**Created:** 2026-01-13
**Type:** documentation

# map_generator_integration_complete
...
Converts generated map format to a processed format with derived metadata (e.g., `mapOriginX`, `mapNativeWidth`).

### ``MapSaveToWorldBox``

Handles the upload of processed maps to `/api/maps/upload`, updates world metadata, and triggers display in `WorldMap2D`.

### ``handleSave` (in `MapsStage`)`

Orchestrates the save workflow—processing, uploading, and clearing the generated map to reflect the saved version.

### ``MapStoryIntegrationBox``

Links map-generated cities to location cards and timeline events via coordinate synchronization.
...
> ``ui-beta/src/boxes/maps/map_process_box.js``
> ``ui-beta/src/boxes/maps/map_save_to_world_box.js``
> ``ui-beta/src/components/stages/MapsStage.vue``
> ``ui-beta/src/boxes/maps/map_story_integration_box.js``
> ``/api/maps/upload``
> ``WorldMap2D` (existing viewer component)`
> ``MapRenderBox``
> ``DataLoadingBox``
> ``MapCoordinateBox`.`
