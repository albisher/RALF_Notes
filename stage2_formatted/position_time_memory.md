**Tags:** #storytelling, #memory-management, #position-tracking, #time-aware, #context-aware, #session-management
**Created:** 2026-01-13
**Type:** documentation-research

# position_time_memory
...
Dispatches operations ("load_variables", "save_context", "clear") to subroutines.

### ``_load_memory_variables()``

Fetches and reconstructs the `StoryContext` from storage.

### ``_save_context()``

Updates the context (e.g., appends story text) and commits changes to storage.

### ``_clear()``

Resets the context to defaults and clears storage.
...
- [[`MemoryStorageBox`]]
- [[`StoryContext`]]
- [[`Box` (core components for persistence and context management).]]
