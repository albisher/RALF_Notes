...
Orchestrates the entire screenshot workflow, including browser launch, navigation, login, and screenshot capture.

### `puppeteer.launch()`

Initializes a headless Chrome browser with security flags (`--no-sandbox`, `--disable-setuid-sandbox`).

### `page.screenshot()`

Captures full-page screenshots at each stage with dynamic filenames.

### `page.goto()`

Navigates to specified URLs with a `networkidle2` wait condition.
