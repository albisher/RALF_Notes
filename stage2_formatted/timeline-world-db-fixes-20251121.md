**Tags:** #database-fixes, #ui-ux-improvements, #web-routes, #event-handling, #web-sockets, #modal-components, #deep-linking, #user-experience
**Created:** 2026-01-14
**Type:** documentation
...
Manages URL hash changes for timeline navigation.

### `switchStage`

Validates and updates stage URLs to support both `#stage` and `#/stage` formats.

### `onWorldTypeChange`

Handles dropdown event changes in `CreateWorldModal.vue` with async validation.

### `handleCreate`

Validates JSON metadata for world creation in `CreateWorldModal.vue`.
...
> ``vue``
> ``vue-router``
> ``WebSocket` library (e.g.`
> ``ws`)`
> ``ui-beta` component library (includes `WorkflowPage.vue` and `CreateWorldModal.vue`).`
