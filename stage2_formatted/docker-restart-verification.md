**Tags:** #docker, #verification, #web-development, #cesium, #frontend-backend, #debugging
**Created:** 2026-01-12
**Type:** code-notes
...
Logs status of view checks across different layout modes.

### ``initCesium2DTopViewer``

Initializes the Cesium 2D top viewer (fixed in multiple locations).

### `Socket.IO Events`

Emitted by the backend (`status-update`, `drones-update`, `visualization-update`).

### `Nginx Container`

Serves frontend files at `http://localhost:5000`.
...
1. **Docker Compose Restart**:
   ...
   ```
   Run in the project’s `docker` directory to restart containers (`hmrs-session-db`, `hmrs-simulator`, `hmrs-backend`, `hmrs-frontend`).
...
> `Docker Compose`
> `Socket.IO`
> `Cesium.js (frontend visualization library)`
> `Nginx (frontend proxy).`

## Related

- [[Dockerfile for `hmrs-backend`]]
- [[docker-compose]]
