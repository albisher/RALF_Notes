**Tags:** #simulation, #session-management, #replay-data, #drone-automation, #api-integration
**Created:** 2026-01-13
**Type:** code-notes
...
Attempts to gracefully stop the simulation via `/api/stop`; falls back to `/api/reset` if needed.

### `reset_simulation()`

Forces a clean reset of the simulation state via `/api/reset` and validates success.

### `create_new_session_with_replay_data()`

Orchestrates session creation, drone spawning, and data generation (partial; further logic omitted).
...
- [[simulation_api_reference]]
- [[drone_automation_guide]]
