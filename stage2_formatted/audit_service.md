**Tags:** #audit-logging, #database-events, #flask, #sqlalchemy, #transaction-management
**Created:** 2026-01-13
...
Registers SQLAlchemy hooks for all tracked entities.

### ``_log_element_change(element, action)``

Logs changes to `WorldElement` instances.

### ``_log_world_change(world, action)``

Logs changes to `World` instances.

### ``_log_timeline_change(timeline, action)``

Logs changes to `Timeline` instances.

### ``_serialize_element(element)``

Converts a `WorldElement` to a dictionary for audit storage.

### ``_get_element_changes(element)``

Extracts diffs between current and previous states (simplified).

### ``_get_world_changes(world)``

Extracts diffs for `World` objects.
...
> For updates, `_get_element_changes()`/`_get_world_changes()` uses a simplified diffing approach. A production system might need SQLAlchemy’s versioning or custom history tracking for full accuracy.
