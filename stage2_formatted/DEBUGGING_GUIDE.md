**Tags:** #debugging, #drones, #simulation, #docker, #waypoint_generation, #random_seed, #exploration_box
**Created:** 2026-01-13
**Type:** documentation
...
Generates dynamic waypoints; failure returns `None`, triggering fallback spiral.

### ``BOXES_AVAILABLE``

Boolean flag indicating if exploration boxes (e.g., `ExplorationEnhancementBox`) are loaded; `False` forces fixed waypoints.

### ``use_enhanced_exploration``

Determines if frontier-based waypoints are used; `False` defaults to fixed paths.

### `Random seed generator`

Uses `time.time()`, `os.getpid()`, and `time.perf_counter()` to produce unique seeds per run.
...
- Python libraries: `os``
> ``time``
> `custom simulation modules (`simulation/swarm/boxes/exploration_enhancement_box.py`)`
