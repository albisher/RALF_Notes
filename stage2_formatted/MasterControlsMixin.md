**Tags:** #Vue, #Mixin, #MasterControls, #Simulation, #DataManagement, #APIIntegration
**Created:** 2026-01-13
**Type:** code-notes
...
Asynchronously fetches and merges latest master control data from `MasterControlsService` into the component’s `masterControls` object.

### ``saveMasterControls()``

Asynchronously saves the current `masterControls` state to the service, triggering success/error notifications if configured.

### ``created()``

Initializes the `MasterControlsService` with the global API endpoint and loads default drone brands if the service is ready.
...
> ``MasterControlsService` (local)`
> ``window.apiCommunicationBox`/`window.apiBox` (global)`
> ``window.loggingBox` (optional)`
> ``showNotification` (optional callback).`
