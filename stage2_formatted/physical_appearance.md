**Tags:** #data-generation, #hashing, #physical-appearance, #dataclasses, #extensible-system
**Created:** 2026-01-13
**Type:** code-library
...
str) -> str`**: Generates a SHA-256 hash from a string input.

### ``derive_int_from_hash(input_hash`

str, salt: str, max_value: int) -> int`**: Derives a deterministic integer from a hash and salt, constrained by `max_value`.

### ``derive_float_from_hash(input_hash`

str, salt: str) -> float`**: Derives a deterministic float (0.0–1.0) from a hash and salt.

### ``generate_appearance_hash(character_id`

str, appearance_type: str, world_id: Optional[str] = None) -> str`**: Combines inputs to produce a unique appearance hash for linking.

### ``determine_appearance_compatibility(character_data`

Dict[str, Any], appearance_type: str) -> Dict[str, Any]`**: Evaluates trait compatibility between a character and an appearance type.

### ``PhysicalAppearanceData``

Dataclass storing metadata for linking character appearances (e.g., `character_id`, `appearance_hash`, `appearance_data`).
...
> ``monsters``
> ``orks``
> ``giants` (submodules under `Creatures`).`

## Related

- [[`character_generation_system`]]
- [[`appearance_registration`]]
