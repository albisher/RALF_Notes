**Tags:** #logging, #event-handling, #frontend-development, #data-operations, #user-interactions, #system-monitoring, #api-integration, #error-handling
**Created:** 2026-01-13
**Type:** code-notes

# notification-logs-to-proper-logging
...
### ``window.loggingBox.log()``

Core logging function with structured metadata for all events.

### ``type` and `action` fields`

Categorize events (e.g., `building_operation`, `user_interaction`) and specify actions (e.g., `save_building`, `get_current_location`).

### `Contextual data inclusion`

Building IDs, coordinates, error details, or warnings for debugging/tracing.

### `Panel-specific routing`

Logs for user interactions appear in the "View & Usage Logs" panel.
...
> ``window.loggingBox` (custom frontend utility for logging)`
> ``showNotification()` (legacy notification system)`
> `external libraries for geolocation (e.g.`
> ``Geolocation API`)`
> `and API calls for weather data.`

## Related

- [[logging-system-design]]
- [[frontend-notification-migration-guide]]
