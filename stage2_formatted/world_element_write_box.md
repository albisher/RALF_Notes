**Tags:** #database, #data-structure, #validation, #authentication, #transaction, #ORM, #logging
**Created:** 2026-01-13
**Type:** documentation

# world_element_write_box
...
BoxInput) -> BoxOutput`**: Orchestrates the entire workflow—validation, database insertion, and result serialization.

### ``__init__(name='world_element_write')``

Initializes the box with a default name and description.

### ``WorldElement` model`

Database model for storing element metadata (e.g., `name`, `element_type`, `data`).
