**Tags:** #test, #ui, #replay, #session, #api, #debugging
**Created:** 2026-01-12
**Type:** code-notes
...
Validates if the session ID exists in the API response.

### `Session Selection`

Attempts to select the session via a POST request to verify API interaction.

### `Replay Data Loading`

Fetches replay data via a GET request to confirm availability.

### `File Inspection`

Checks the local `motion_history.json` file for drone motion data manually.
