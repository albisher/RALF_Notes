**Tags:** #object-oriented-design, #exploration-algorithms, #lidar-processing, #path-planning, #autonomous-robotics, #3d-visualization, #box-architecture, #waypoint-generation, #adaptive-exploration, #lidar-drone
**Created:** 2026-01-13
**Type:** code-notes
...
### ``_generate_with_boxes()``

Uses existing `FrontierExplorerBox` and `AdvancedLiDARBox` if available.

### ``_generate_simple()``

Fallback to basic BFS clustering if specialized boxes fail.

### ``_cluster_frontiers()``

Clustering algorithm for frontier-based waypoint selection.
...
Checks path feasibility with safety margins.

### ``_plan_avoidance_path()``

Generates paths avoiding detected obstacles.
