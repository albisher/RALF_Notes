**Tags:** #map-viewing, #optimization, #2D-viewer, #coordinate-centering, #box-pattern, #world-map, #single-source-truth
**Created:** 2026-01-13
**Type:** code-notes

# map_view_optimization_box_20251206
...
### ``MapViewOptimizationBox.optimize()``

Computes optimal scale, offset, and viewBox settings from map vertex data.

### ``WorldMap2D.vue.renderMap()``

Integrates the box to apply optimized view settings, removing legacy positioning logic.

### ``TopTimelineMap.vue.renderMap()``

Uses the box for consistent centering on content (not (0,0)) for smaller maps.
...
> ``ui-beta/src/boxes/maps/map_view_optimization_box.js``
> ``ui-beta/src/components/common/WorldMap2D.vue``
> ``ui-beta/src/components/common/TopTimelineMap.vue``
> `Box interface pattern (internal).`

## Related

- [[`Map View Optimization Box Implementation`]]
- [[`WorldMap2D]]
- [[`TopTimelineMap]]

>[!INFO] Critical Dependency
> The `MapViewOptimizationBox` must be used exclusively for map positioning—no other logic should override its outputs.
