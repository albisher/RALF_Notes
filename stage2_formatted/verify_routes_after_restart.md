**Tags:** #automation, #post-restart-validation, #session-endpoint-testing, #curl, #health-check
**Created:** 2026-01-13
**Type:** code-notes
...
Delays execution until the server is ready.

### ``curl` + `grep`/`python3` parsing`

Extracts HTTP status codes and JSON fields (e.g., session count, `replay_available`).

### `Session-specific tests`

Tests `/api/sessions/<id>/select` and `/api/sessions/<id>/replay` with a hardcoded ID (`202512131159`).

### `Health check`

Validates `/api/health` for a "status.ok" response.
