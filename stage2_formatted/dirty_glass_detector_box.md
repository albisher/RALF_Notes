**Tags:** #sensor_processing, #image_analysis, #detection_algorithms, #computer_vision, #drone_automation
**Created:** 2026-01-13
**Type:** code-notes
...
Initializes the detector with configurable thresholds for dirtiness and minimum surface area.

### ``detect_from_camera``

Processes camera data (RGB image) to detect dirty surfaces by analyzing brightness, contrast, and variance in grid cells.

### ``_project_cell_to_world``

(Internal) Simulates projecting a detected cell’s position from camera coordinates to 3D world coordinates (not fully implemented in snippet).
