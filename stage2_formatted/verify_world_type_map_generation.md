**Tags:** #automated-testing, #puppeteer, #ui-verification, #world-generation, #game-dev
**Created:** 2026-01-13
**Type:** code-test

# verify_world_type_map_generation
...
Authenticates the user with test credentials or checks if already logged in.

### `selectWorld`

Attempts to find and select a world from available options via multiple selector patterns.

### `getWorldType`

Extracts the current world type from UI elements or page text using regex and fallback methods.

### `generateMapForWorldType`

Orchestrates map generation for a specified world type, takes initial screenshots, and handles hash input.

### `sleep`

Utility async function for controlled delays.
