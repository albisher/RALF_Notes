**Tags:** #drone-weather, #simulation, #gps-integration, #weather-api, #onboard-sensors
**Created:** 2026-01-13
**Type:** code-notes
...
Initializes weather state (calm conditions by default) and configures fallback modes (API vs onboard sensors). Sets up GPS coordinates for API integration if provided.

### ``_update_forecast_from_api``

(Internal) Fetches external weather data from a hypothetical `WeatherAPIIntegration` module.

### ``wind_speeds``

Dictionary mapping `WeatherCondition` enums to min/max wind speed ranges (e.g., `STORM` = 15–25 m/s).
