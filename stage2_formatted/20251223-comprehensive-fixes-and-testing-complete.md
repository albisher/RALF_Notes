**Tags:** #Vue.js, #Cesium, #Reactivity, #DOM-rendering, #Vue-3, #Frontend-Fixes, #Conditional-Rendering, #Error-Handling
**Created:** 2026-01-12
**Type:** code-notes
...
### ``cesium-2d-top-viewer`/`cesium-3d-viewer` container rendering`

Changed from `v-if` to `v-show` to ensure persistent DOM presence.

### `Error handling for command processing`

Added try-except block around `drone.receive_command()` to prevent HTTP 500 errors.
...
> ``simulation/frontend` package dependencies.`
