**Tags:** #timestamp-fix, #logging, #frontend-backend-integration, #simulation, #debugging
**Created:** 2026-01-12
**Type:** code-notes
...
### ``formatLogTime(simTime, timestamp)``

Formats timestamps dynamically, preferring simulation time when available, falling back to real timestamps.

### ``logger.log()` (Python backend)`

Modified to include `sim_time` in log entries when simulation is active.

### ``logs-sidebar-component.js``

Updated template to render formatted timestamps in the UI.
...
> ``LoggingBox` (frontend logging system)`
> ``Socket.IO` (for real-time connection tracking)`
> ``Python logging` (backend logging)`
> ``JavaScript Date` (for timestamp conversion).`
