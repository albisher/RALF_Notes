...
## 6. Tag Management & Refinement

RALF Note provides a powerful Tag Refinement System to help you maintain consistent and organized tags across your generated documentation. This system allows you to analyze existing tags, get LLM-powered suggestions for consolidation, and automatically apply those changes.

### 6.1. Analyzing Tags (`ralf-notes tags analyze`)

This command scans your markdown files, extracts all tags, analyzes them for patterns (like compound tags or similar names), and uses an LLM to suggest refinements.

```bash
ralf-notes tags analyze [TARGET_DIR] --output guide.json [--model <MODEL_NAME>] [--max-tags <NUMBER>] [--quiet]
```

-   `[TARGET_DIR]`: Optional. The directory containing your markdown files (defaults to your configured target directory).
-   `--output <JSON_FILE>`: Specifies the output JSON file for the tag refinement guide (default: `tag_refinement_guide.json`).
-   `--model <MODEL_NAME>`: Optional. Overrides the LLM model used for generating refinement suggestions.
-   `--max-tags <NUMBER>`: Optional. Maximum number of most frequent tags to send to the LLM for analysis (default: 100).
-   `--quiet`: Suppresses verbose terminal output.

**Example Workflow:**
```bash
ralf-notes tags analyze ~/ObsidianVault/CodeNotes --output my_tag_guide.json
```
This will generate `my_tag_guide.json`, which you should review before applying.

### 6.2. Applying Tag Refinements (`ralf-notes tags apply`)

Once you have reviewed and approved a tag refinement guide (a JSON file generated by `analyze`), you can use this command to apply the changes to your markdown files.

```bash
ralf-notes tags apply [TARGET_DIR] --guide <GUIDE_JSON_FILE> [--dry-run] [--no-backup] [--quiet]
```

-   `[TARGET_DIR]`: Optional. The directory containing your markdown files (defaults to your configured target directory).
-   `--guide <JSON_FILE>`: **Required**. Path to the tag refinement guide JSON file.
-   `--dry-run`: Optional. Shows what changes would be made without actually writing to files. Highly recommended for previewing.
-   `--no-backup`: Optional. By default, a backup of the `TARGET_DIR` is created before applying changes. Use this flag to disable backup.
-   `--quiet`: Suppresses verbose terminal output.

**Example Workflow:**
```bash
# Preview changes first
ralf-notes tags apply ~/ObsidianVault/CodeNotes --guide my_tag_guide.json --dry-run

# If preview looks good, apply changes with backup
ralf-notes tags apply ~/ObsidianVault/CodeNotes --guide my_tag_guide.json
```

### 6.3. Viewing Tag Statistics (`ralf-notes tags stats`)

This command provides an overview of the tags used in your markdown files, including frequency and associated files.

```bash
ralf-notes tags stats [TARGET_DIR] [--quiet]
```
...
**Example Workflow:**
```bash
ralf-notes tags stats ~/ObsidianVault/CodeNotes
```
...
-   **Use `--dry-run`:** Always use `--dry-run` with `generate` or `format-initial` or `tags apply` when experimenting to see the expected output without modifying files.
...
-   **Refine Tags Regularly:** Use the Tag Refinement System to keep your tags clean and consistent as your knowledge base grows.
